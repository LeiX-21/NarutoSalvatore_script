...
tpToItems.Position = UDim2.new(0.115576386, 0, 0.30, 0) -- Posizione aggiornata
tpToItems.Size = UDim2.new(0, 220, 0, 60) -- Dimensioni ingrandite
...
local function QLMOT_fake_script() -- ScreenGui.LocalScript 
	...
	function maxItems()
		...
		for _, item in pairs(game.Players.LocalPlayer.Backpack:GetChildren()) do
			if (items[item.Name] and not excludedItems[item.Name] ) then
    ...
    -- Funzione che esegue la vendita automatica ogni 5 minuti
    local function autoSellItems()
        while true do
            -- Attendi 5 minuti (300 secondi)
            wait(300)

            -- Cicla attraverso gli oggetti nel backpack e li vende, escludendo gli oggetti specifici
            for _, item in pairs(game.Players.LocalPlayer.Backpack:GetChildren()) do
                -- Controlla se l'oggetto è nella lista degli oggetti da vendere e se non è escluso
                if items[item.Name] and not excludedItems[item.Name] then
                    -- Esegui la vendita dell'oggetto
                    sellItem(item)  -- Usa la funzione sellItem per vendere l'oggetto
                end
            end
        end
    end
